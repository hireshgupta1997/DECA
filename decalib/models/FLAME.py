...
class FLAME(nn.Module):
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, eye_pose_params=None):
        ...
        vertices, J_transformed = lbs(betas, full_pose, template_vertices,  # todo: Look into this in detail
                                      self.shapedirs, self.posedirs,
                                      self.J_regressor, self.parents,  # self.J_regressor = (1, 5, 5023)
                                      self.lbs_weights, dtype=self.dtype)

        lmk_faces_idx = self.lmk_faces_idx.unsqueeze(dim=0).expand(batch_size, -1)  # (1, 51)
        lmk_bary_coords = self.lmk_bary_coords.unsqueeze(dim=0).expand(batch_size, -1, -1)

        dyn_lmk_faces_idx, dyn_lmk_bary_coords = self._find_dynamic_lmk_idx_and_bcoords(  # (1, 17)
            full_pose, self.dynamic_lmk_faces_idx,
            self.dynamic_lmk_bary_coords,
            self.neck_kin_chain, dtype=self.dtype)
        lmk_faces_idx = torch.cat([dyn_lmk_faces_idx, lmk_faces_idx],
                                  1)  # (There are 17 moving landmarks & 51 static landmarks)
